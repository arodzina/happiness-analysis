...
print(dim(data))        # Wymiary oryginalnego zbioru danych
print(dim(data_clean))  # Wymiary zbioru po usunięciu braków

print(dim(data)[1] - dim(data_clean)[1])  # Liczba usuniętych wierszy
```
...
# Lista zmiennych, które chcesz analizować (wszystkie numeryczne oprócz 'year')
vars_to_plot <- names(data_clean)[sapply(data_clean, is.numeric) & names(data_clean) != "year"]

# Pętla dla każdej zmiennej
for (var_name in vars_to_plot) {
  
  variable <- data_clean[[var_name]]
  ...
  # Histogram
  histogram <- ggplot(data_clean, aes(x = .data[[var_name]])) + 
