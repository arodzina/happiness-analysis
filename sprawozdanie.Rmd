...
dane_finalne$binarna_2020 <- ifelse(dane_finalne$year == 2021, 1, 0)
dane_finalne <- dane_finalne %>% select(-binarna)

model60 <- lm(`Life.Ladder` ~ `Social.support` +
              `Healthy.life.expectancy.at.birth` +
               `Perceptions.of.corruption` +`Freedom.to.make.life.choices` +Country.name,
              data = train)
summary(model60)
vif(model60)
```
...
# Wybór danych numerycznych i obliczenie korelacji
num_data <- dane_finalne %>% select_if(is.numeric)
cor_matrix <- cor(num_data, use = "complete.obs")
cor_melted <- melt(cor_matrix)

# Heatmapa z poprawioną estetyką
ggplot(cor_melted, aes(x = Var1, y = Var2, fill = value)) +
  geom_tile(color = "white", linewidth = 0.5) +  # cieńsze krawędzie
  scale_fill_gradient2(
    low = "#2166AC",  # ciemnoniebieski
    mid = "white",
    high = "#B2182B", # ciemnoczerwony
    midpoint = 0,
    limit = c(-1, 1),
    name = "Korelacja"
  ) +
  geom_text(aes(label = ifelse(Var1 == Var2, "", sprintf("%.2f", value))), 
            size = 3.5, color = "black") +  # bez cyfr na przekątnej
...
model3 <- lm(`Life.Ladder` ~ `Social.support` +
              `Healthy.life.expectancy.at.birth` +
               `Perceptions.of.corruption` +`Freedom.to.make.life.choices` +`binarna_2020`,
              data = train)
summary(model3)
plot(model3)
vif(model3)
